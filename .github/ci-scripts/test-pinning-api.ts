 ...
 *   cd pinning-api-v3 && pnpm tsx ../.github/ci-scripts/test-pinning-api.ts [worker-base-url]
 *   pnpm --filter pinning-api-v3 test:deployed
 *   pnpm --filter pinning-api-v3 test:deployed https://pinning.centrifuge.io
 *
 * Reads PINATA_JWT from pinning-api-v3/.env (local) or process.env (CI).
 */
...
// .env lives in pinning-api-v3/ — two levels up from .github/ci-scripts/
config({ path: resolve(__dirname, '../../pinning-api-v3/.env') })

const BASE_URL = process.argv[2] ?? 'https://pinning-api-dev.kf-dev.workers.dev'
